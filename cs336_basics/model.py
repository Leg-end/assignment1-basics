...
class BasicsTransformerLM(nn.Module):
    ...
    def forward(self, x: Int[Tensor, " ... sequence_length"],
                y: Int[Tensor, " ... sequence_length"] | None=None) -> Float[Tensor, " ... sequence_length vocab_size"]:
        """
        Args:
        ...
        if y is not None:
            ...
            loss = F.cross_entropy(logits.view(-1, self.vocab_size), y.view(-1), ignore_index=-1)
        else:
